...
- **InPrivate 隐私模式** — 在 Edge 隐私模式下打开页面，不保留任何浏览记录、Cookie 或缓存；使用独立用户数据目录，不污染主浏览器配置；禁用扩展、同步、翻译等干扰功能；启用并行下载
...
- **InPrivate mode** — Opens pages in Edge InPrivate mode; no browsing history, cookies, or cache are retained. Uses a separate user data directory to avoid polluting your main browser profile; disables extensions, sync, translation, and other distractions; enables parallel downloading

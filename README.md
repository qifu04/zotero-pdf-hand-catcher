...
- **隔离 Edge + 机构认证复用** — 使用独立用户数据目录启动 Edge，复制系统 Edge 的 Cookie 以保留机构/校园网登录状态，同时自动剥离 Cloudflare 相关 Cookie（`cf_clearance`、`__cf_bm` 等）避免指纹不匹配触发反爬；加入反自动化检测措施，降低被识别为机器人的概率
...
- **Isolated Edge + institutional auth reuse** — Launches Edge with a separate user data directory, copies cookies from your system Edge to preserve institutional/campus login sessions, and automatically strips Cloudflare cookies (`cf_clearance`, `__cf_bm`, etc.) to avoid fingerprint-mismatch bot detection. Includes anti-automation-detection measures to reduce the chance of being flagged as a bot
